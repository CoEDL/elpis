...
elan.general_setting(
    key="tier_name", ui_format="select", display_name="name", options=[], shown=False
)
elan.general_setting(
    key="tier_type", ui_format="select", display_name="type", options=[], shown=False
)
elan.general_setting(
    key="tier_order", ui_format="select", display_name="order", options=[], shown=False
)

elan.general_setting_title(title="punctuation")
elan.general_setting(
    key="punctuation_to_explode_by",
    ui_format="text",
    display_name="explosion",
    default=string.punctuation + ",…‘’“”°",
)
elan.general_setting(
    key="punctuation_to_collapse_by",
    ui_format="text",
...
elan.general_setting(
    key="special_cases", ui_format="textarea", display_name="words", default="<silence>"
)
elan.general_setting(
    key="translation_tags", ui_format="textarea", display_name="tags", default="@eng@"
)
...
def update_ui(file_paths: List[Path], ui):
    ...
    for eaf_path in eaf_paths:
        ...
        for tier_type in list(input_eaf.get_linguistic_type_names()):
            ...
            tier_ids: List[str] = input_eaf.get_tier_ids_for_linguistic_type(tier_type)
