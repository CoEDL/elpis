...
def get_chunks(audio_path: str, method: str, parameter: float) -> List[Tuple[float, float]]:
    """
    Chunk voice sections from audio data extracted from an audio path with the chosen method
    (with its parameter).
...
def read_audio_path(audio_path: str) -> Dict[str, Any]:
    ...
    :param audio_path: audio file path.
    :return: dict of data: *signal* (array of floats), sampling *rate* (integer),
    and *top db* (float).
    """
...
def find_best_threshold(audio_data: Dict[str, Any], method: str, parameter: float) -> float:
    """
    Find the best threshold of audio data for the chosen method and parameter. For all methods, if
    the result is higher than top db, it is lowered to the latter.

    :param audio_data: dict of audio data.
    :param method: chunking method: *duration* (highest threshold where all voice sections are
    shorter than duration), *offset* (top db − offset), *threshold* (fixed threshold).
    :param parameter: parameter of the chosen method.
    ...
    if method == "duration":
        ...
        thresholds = [division["threshold"] for division in continuum if
                      division["size"] == division["limited size"]]
        threshold = max(thresholds) if thresholds else audio_data["top db"]
    elif method == "offset":
        threshold = audio_data["top db"] - parameter if parameter < audio_data["top db"] \
...
def get_continuum(audio_data: Dict[str, Any], max_duration: float, size: int = 20) \
        -> List[Dict[str, Any]]:
    """
    Calculate the continuum (of chosen size) of voice sections (timestamps) depending on max
    duration limit.

    :param audio_data: dict of audio data.
    :param max_duration: duration limit (in seconds).
    :param size:
    :return: a list of dicts of useful data: *timestamps* (in seconds) of voice sections,
    *threshold*, *durations* of voice sections, *size* (number of voice sections) and
    *limited size* (number of voice sections shorter than duration).
    """
...
def get_voice_sections(audio_data: Dict[str, Any], threshold: float) -> List[Tuple[float, float]]:
    ...
    time_voice_sections = [(start / audio_data["rate"], end / audio_data["rate"]) for
                           index, (start, end) in enumerate(frame_voice_sections, 1)]
    return time_voice_sections
