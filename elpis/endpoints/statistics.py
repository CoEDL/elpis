...
from .utils.statistics import parse_grapheme_frequency_from_dataset, get_length_of_wav, generate_word_sankey, generate_grapheme_sankey
...
def graphemes(pron_dict: PronDict, dataset: Dataset, file_name: str = None):
    ...
    phrases = []
    if file_name is not None:
        phrases = [annotation['transcript']
                   for annotation in annotations if annotation['audio_file_name'] == file_name + ".wav"]
    else:
...
def count(dataset: Dataset, file_name: str = None):
    ...
    transcriptions = []
    if file_name is not None:
        transcriptions = [t for t in load_json_file(
            f'{dataset.pathto.annotation_json}') if t['audio_file_name'] == (file_name + ".wav")]
    else:
...
def annotated(dataset: Dataset, file_name: str = None):
    ...
    if file_name is not None:
        ...
        for transcription in annotations:
            if transcription['audio_file_name'] == (file_name + ".wav"):
                annotated_time += transcription['stop_ms'] - \
                    transcription['start_ms']
        return jsonify({"status": 200,
                        "annotated": annotated_time / float(total_time * 1000)})
    else:
        for file_name in [f for f in dataset.files if f.endswith(".wav")]:
            total_time += get_length_of_wav(
                dataset.pathto.original.joinpath(file_name))
        for transcription in annotations:
            annotated_time += transcription['stop_ms'] - \
                transcription['start_ms']
    return jsonify({
...
def sankey_word(dataset: Dataset, file_name: str = None):
    ...
    phrases = []
    if file_name is not None:
        phrases = [annotation["transcript"] for annotation in annotations if annotation['audio_file_name'] == file_name + ".wav" ]
    else:
        phrases = [annotation["transcript"] for annotation in annotations]
    return jsonify({
        "status": 200,
        "data": {
            "sankey": generate_word_sankey(phrases)
        }
    })
...
def sankey_grapheme(pron_dict: PronDict, dataset: Dataset, file_name: str = None):
    ...
    return jsonify({
        "status": 200,
        "data": {
            "sankey": generate_grapheme_sankey(phrases, graphemes)
        }
    })
...
def swarm_plot_files(dataset: Dataset):
    ...
    for annotation in annotations:
        ...
        if file_name not in swarm_plot:
            swarm_plot[file_name] = {
                "file": file_name,
                "group": "Files",
                "length": get_length_of_wav(dataset.pathto.original.joinpath(annotation["audio_file_name"])),
    ...
    return jsonify({
        "status": 200,
        "data": {
            "swarmplot": list(swarm_plot.values())
        }
    })
